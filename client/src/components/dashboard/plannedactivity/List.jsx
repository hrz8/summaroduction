...
class ListStandard extends Component {
  ...
  drawTable = async (url = `http://${process.env.REACT_APP_API_URL || 'localhost'}:3029/planned-activity`) => {
    try {
  ...
  handleSearch = async () => {
    let queryString = `http://${process.env.REACT_APP_API_URL || 'localhost'}:3029/planned-activity`;
    if (this.state.q) {
  ...
  handleDelete = async e => {
    if (window.confirm(`IMPORTANT: MENGHAPUS DATA INI DAPAT MEMBUAT ERROR DATA PRODUCTION. SEBELUM MELANJUTKAN, PASTIKAN TERLEBIH DAHULU TIDAK ADA DATA PADA PRODUCTION YANG MEMAKAI DATA INI SEBAGAI DATA PLANNED ACTIVITY}!!!\ningin melanjutkan?`)) {
      if (window.confirm(`Hapus data dengan id ${e.target.dataset.id} (${e.target.dataset.name}) ?`)) {
        try {
          const deleted = await axios_delete(`http://${process.env.REACT_APP_API_URL || 'localhost'}:3029/planned-activity/${e.target.dataset.id}`, this.props.store.auth.access_token)
  ...
  render = () => {
    const columns = [
      ...
      {
        Header: 'Default Waktu (menit)',
        accessor: 'minuteDefault',
        width: 200
      },
      {
        Header: 'Keterangan',
      ...
      },
      {
        ...
        Cell: ({ original }) => (
          <>
            <Link to={{ pathname: `/dashboard/planned-activity/detail/${original.id}` }}
              className="btn btn-cc btn-cc-primary btn-cc-radius-normal p-1 mb-1">
              <FontAwesomeIcon icon={faInfoCircle} />&nbsp;Detil
            </Link>
            <Link to={{ pathname: `/dashboard/planned-activity/edit/${original.id}` }}
              className="btn btn-cc btn-cc-primary btn-cc-radius-normal p-1 mb-1">
    ...
    return (
      <Card title={"Planned Activity"} col={12}>
          ...
          <>
            <div className="row align-items-center">
              ...
              <div className="col-md-3 ml-md-auto text-center text-md-right">
                <Link
                  to={{ pathname: `/dashboard/planned-activity/add` }}
